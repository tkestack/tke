...
const monitorFile = "monitor"

func TestLoad(t *testing.T) {
	cases := []struct {
		desc   string
		file   *string
		expect *monitorconfig.MonitorConfiguration
		err    string
	}{
		// missing file
		{
		...
		{
			"missing kind",
			newString(`{"apiVersion":"monitor.config.tkestack.io/v1"}`),
			nil,
			"failed to decode",
		},
		{
			"missing version",
			newString(`{"kind":"MonitorConfiguration"}`),
			nil,
			"failed to decode",
		},
		{
			"unregistered kind",
			newString(`{"kind":"BogusKind","apiVersion":"monitor.config.tkestack.io/v1"}`),
			nil,
			"failed to decode",
		},
		{
			"unregistered version",
			newString(`{"kind":"MonitorConfiguration","apiVersion":"bogusversion"}`),
			nil,
			"failed to decode",
		},
...
apiVersion: monitor.config.tkestack.io/v1`),
		...
		{
			"default from json",
			newString(`{"kind":"MonitorConfiguration","apiVersion":"monitor.config.tkestack.io/v1"}`),
			newConfig(t),
			"",
		},
	}
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			fs := utilfs.NewFakeFs()
			path := filepath.Join(configDir, monitorFile)
			if c.file != nil {
...
func newConfig(t *testing.T) *monitorconfig.MonitorConfiguration {
	monitorScheme, _, err := monitorscheme.NewSchemeAndCodecs()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// get the built-in default configuration
	external := &monitorconfigv1.MonitorConfiguration{}
	monitorScheme.Default(external)
	kc := &monitorconfig.MonitorConfiguration{}
	err = monitorScheme.Convert(external, kc, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	return kc
