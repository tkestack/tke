...
func (h *processor) Update(ctx context.Context, clusterName string, alertValue string, entity *alertconfig.Route) error {
	h.Lock()
	defer h.Unlock()
	...
	log.Infof("Start to update route %s", alertValue)

	_, err = routeOp.UpdateRoute(alertValue, entity)
	if err != nil {
		return errors.Wrapf(err, "failed to update route %s", alertValue)
	}
