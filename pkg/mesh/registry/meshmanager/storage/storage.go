...
// Storage includes storage for meshmanager and all sub resources.
type Storage struct {
	MeshManager *REST
	Status      *StatusREST
}

// NewStorage returns a Storage object that will work against meshmanager.
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, privilegedUsername string) *Storage {
	strategy := meshmanagerstrategy.NewStrategy()
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &mesh.MeshManager{} },
		NewListFunc:              func() runtime.Object { return &mesh.MeshManagerList{} },
		DefaultQualifiedResource: mesh.Resource("meshmanagers"),
		PredicateFunc:            meshmanagerstrategy.MatchMeshManager,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create meshmanager etcd rest storage", log.Err(err))
	}
...
// REST implements a RESTStorage for meshmanager against etcd.
type REST struct {
	*registry.Store
...
// StatusREST implements the REST endpoint for changing the status of a meshmanager.
