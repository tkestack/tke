...
func CreateOrUpdateDeployment(client clientset.Interface, deploy *appsv1.Deployment) error {
...
func CreateOrUpdateDaemonSet(client clientset.Interface, deploy *appsv1.DaemonSet) error {
	if _, err := client.AppsV1().DaemonSets(deploy.ObjectMeta.Namespace).Create(deploy); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Wrap(err, "unable to create daemonset")
		}

		if _, err := client.AppsV1().DaemonSets(deploy.ObjectMeta.Namespace).Update(deploy); err != nil {
