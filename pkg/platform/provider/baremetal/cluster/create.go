...
func (p *Provider) EnsureLoadBalance(ctx context.Context, c *v1.Cluster) error {
	...
	for _, machine := range c.Spec.Machines {
		...
		for _, chain := range chains {
			if c.Spec.Features.IPVS != nil && *c.Spec.Features.IPVS {
				//! see proposal controlplan-ha-loadbalance
				cmd := fmt.Sprintf("iptables -t nat -D %s -d %s -p tcp --dport 6443 -j DNAT --to-destination 10.244.255.1:6443",
					chain, c.Spec.Features.HA.TKEHA.VIP)
				s.Exec(cmd)

				cmd = fmt.Sprintf("iptables -t nat -D %s -d %s -p tcp --dport 6443 -j KUBE-MARK-MASQ",
					chain, c.Spec.Features.HA.TKEHA.VIP)
				s.Exec(cmd)

				cmd = fmt.Sprintf("iptables -t nat -I %s -d %s -p tcp --dport 6443 -j DNAT --to-destination 10.244.255.1:6443",
					chain, c.Spec.Features.HA.TKEHA.VIP)
				s.Exec(cmd)
				...
				log.Info("ipvs mod loadblance created success.", log.String("name", c.Cluster.Name), log.String("node", machine.IP), log.String("chain", chain))
				...
				cmd = fmt.Sprintf("iptables -t nat -D %s -d %s -p tcp --dport 6443 -j KUBE-SVC-NPX46M4PTMTKRN6Y",
					chain, c.Spec.Features.HA.TKEHA.VIP)
				s.Exec(cmd)
				...
				//! see proposal controlplan-ha-loadbalance
				cmd = fmt.Sprintf("iptables -t nat -I %s -d %s -p tcp --dport 6443 -j KUBE-SVC-NPX46M4PTMTKRN6Y",
					chain, c.Spec.Features.HA.TKEHA.VIP)
				s.Exec(cmd)
				...
				log.Info("iptable mod loadblance created success.", log.String("name", c.Cluster.Name), log.String("node", machine.IP), log.String("chain", chain))
			}
		}
...
func (p *Provider) EnsureKeepalived(ctx context.Context, c *v1.Cluster) error {
	if c.Spec.Features.HA == nil || c.Spec.Features.HA.TKEHA == nil {
		return nil
	}
	...
	for i, machine := range c.Spec.Machines {
		...
		// node 0 has installed EnsurePrepareForControlplane
		if i == 0 {
			continue
		}

		err = keepalived.Install(s, machine.IP, c.Spec.Features.HA.TKEHA.VIP)
		if err != nil {
			return err
		}
