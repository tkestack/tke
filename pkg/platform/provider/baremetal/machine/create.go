...
func (p *Provider) EncryptSSH(ctx context.Context, machine *platformv1.Machine, cluster *typesv1.Cluster) error {
	keyBytes, err := ioutil.ReadFile(ssh.AESKeyFile)
	if err != nil {
		log.FromContext(ctx).Infof("read aes key failed %v, skip encrypt ssh", err)
		return nil
	}
	return platformutil.EncryptMachineSSH(ctx, p.platformClient, machine, string(keyBytes))
}
