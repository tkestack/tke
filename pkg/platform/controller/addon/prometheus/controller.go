...
func (c *Controller) createPrometheusIfNeeded(key string, cachedPrometheus *cachedPrometheus, prometheus *v1.Prometheus) error {
	...
	case v1.AddonPhaseFailed:
		...
		if _, ok := c.checking.Load(key); !ok {
			...
			go func() {
				defer c.checking.Delete(key)
				wait.PollImmediateUntil(60*time.Second, c.checkPrometheusStatus(prometheus, key, delayTime), c.stopCh)
			}()
		}
	}
	return nil
...
func (c *Controller) installPrometheus(prometheus *v1.Prometheus) error {
	...
	if prometheus.Spec.WithNPD {
		...
		if _, err := kubeClient.CoreV1().ServiceAccounts(metav1.NamespaceSystem).Create(createServiceAccountForNPD()); err != nil {
			return err
		}
		// ClusterRole for node-problem-detector
		if _, err := kubeClient.RbacV1().ClusterRoles().Create(createClusterRoleForNPD()); err != nil {
			return err
		}
		// ClusterRoleBinding for node-problem-detector
		if _, err := kubeClient.RbacV1().ClusterRoleBindings().Create(createClusterRoleBindingForNPD()); err != nil {
			return err
		}
		// DaemonSet for node-problem-detector
		if _, err := kubeClient.AppsV1().DaemonSets(metav1.NamespaceSystem).Create(createDaemonSetForNPD(components)); err != nil {
			return err
		}
...
func (c *Controller) uninstallPrometheus(prometheus *v1.Prometheus, dropData bool) error {
	...
	if prometheus.Spec.WithNPD {
		err = kubeClient.AppsV1().DaemonSets(metav1.NamespaceSystem).Delete(nodeProblemDetectorWorkload, &metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			errs = append(errs, err)
		}
		err = kubeClient.RbacV1().ClusterRoleBindings().Delete(nodeProblemDetectorClusterRoleBinding, &metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			errs = append(errs, err)
		}
		err = kubeClient.RbacV1().ClusterRoles().Delete(nodeProblemDetectorClusterRole, &metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			errs = append(errs, err)
		}
		err = kubeClient.CoreV1().ServiceAccounts(metav1.NamespaceSystem).Delete(nodeProblemDetectorServiceAccount, &metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			errs = append(errs, err)
		}
	}
