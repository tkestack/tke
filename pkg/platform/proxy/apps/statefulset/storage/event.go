...
func (r *EventREST) Get(ctx context.Context, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	if apiclient.ClusterVersionIsBefore19(client) {
		return listEventsByAppsBeta(ctx, client, namespaceName, name, options)
	}
...
func listEventsByApps(ctx context.Context, client *kubernetes.Clientset, namespaceName, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	if err != nil {
		return nil, errors.NewNotFound(appsv1.Resource("statefulsets/events"), name)
	}

	var resultEvents util.EventSlice

	events, errs := getAboutStatefulSetEvents(ctx, client, name, namespaceName, string(statefulSet.UID))
	if len(errs) > 0 {
		return nil, utilerrors.NewAggregate(errs)
	}

	involvedObjectUIDMap := util.GetInvolvedObjectUIDMap(events)
	if v, ok := involvedObjectUIDMap[string(statefulSet.UID)]; ok {
...
func listEventsByAppsBeta(ctx context.Context, client *kubernetes.Clientset, namespaceName, name string, options *metav1.GetOptions) (runtime.Object, error) {
...
// getAboutStatefulSetEvents Query all events in the namespace  or Query the StatefulSet Pod asynchronously
func getAboutStatefulSetEvents(ctx context.Context, client *kubernetes.Clientset, name, namespace, uid string) (util.EventSlice, []error) {
	return util.GetResourcesEvents(ctx, client, namespace, []metav1.ListOptions{
		{
			FieldSelector: fields.AndSelectors(
				...
				fields.OneTermEqualSelector("involvedObject.kind", "StatefulSet")).String(),
			ResourceVersion: "0",
		},
		{
			FieldSelector: fields.AndSelectors(
