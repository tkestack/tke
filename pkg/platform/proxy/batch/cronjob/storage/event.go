...
func (r *EventREST) Get(ctx context.Context, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	// cronjob, err := client.BatchV2alpha1().CronJobs(namespaceName).Get(name, *options)
	// if err != nil {
	// 	log.Error("Failed to get batch v1beta1 cronjob by name", log.BaseName("namespace", namespaceName), log.BaseName("name", name), log.Err(err))
	// 	return nil, errors.NewNotFound(batchV2Alpha1.Resource("cronjobs/events"), name)
	// }

	selector := fields.AndSelectors(
		// fields.OneTermEqualSelector("involvedObject.uid", string(cronjob.UID)),
		fields.OneTermEqualSelector("involvedObject.name", name),
		fields.OneTermEqualSelector("involvedObject.namespace", namespaceName),
