...
import (
	...
	poddisruptionbudgetstorage "tkestack.io/tke/pkg/platform/proxy/policy/poddisruptionbudget/storage"
...
func (s *StorageProvider) v1Beta1Storage(restOptionsGetter generic.RESTOptionsGetter, loopbackClientConfig *restclient.Config) map[string]rest.Storage {
	...
	podDisruptionBudgetStore := poddisruptionbudgetstorage.NewStorageV1Beta1(restOptionsGetter, platformClient)
	podSecurityPolicyStore := podsecuritypolicystorage.NewStorageV1Beta1(restOptionsGetter, platformClient)

	storageMap := map[string]rest.Storage{
		"poddisruptionbudgets":        podDisruptionBudgetStore.PodDisruptionBudget,
		"poddisruptionbudgets/status": podDisruptionBudgetStore.Status,
