...
import (
	...
	"tkestack.io/tke/pkg/platform/registry/logcollector"
...
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, platformClient platforminternalclient.PlatformInterface, privilegedUsername string) *Storage {
	strategy := logcollector.NewStrategy(platformClient)
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &platform.LogCollector{} },
		NewListFunc:              func() runtime.Object { return &platform.LogCollectorList{} },
		DefaultQualifiedResource: platform.Resource("logcollectors"),
		PredicateFunc:            logcollector.MatchLogCollector,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create LogCollector etcd rest storage", log.Err(err))
	}

	statusStore := *store
	statusStore.UpdateStrategy = logcollector.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = logcollector.NewStatusStrategy(strategy)

	return &Storage{
		LogCollector: &REST{store, privilegedUsername},
...
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	logCollector := obj.(*platform.LogCollector)
	if err := util.FilterLogCollector(ctx, logCollector); err != nil {
		return nil, err
	}
...
func (r *REST) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions, listOptions *metainternal.ListOptions) (runtime.Object, error) {
	if !authentication.IsAdministrator(ctx, r.privilegedUsername) {
		return nil, errors.NewMethodNotSupported(platform.Resource("logcollectors"), "delete collection")
	}
