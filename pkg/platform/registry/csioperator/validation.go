...
func ValidateCSIOperator(ctx context.Context, platformClient platforminternalclient.PlatformInterface, csiOperator *platform.CSIOperator) field.ErrorList {
	allErrs := apiMachineryValidation.ValidateObjectMeta(&csiOperator.ObjectMeta, false, ValidateName, field.NewPath("metadata"))
	allErrs = append(allErrs, validation.ValidateCluster(ctx, platformClient, csiOperator.Spec.ClusterName)...)
...
func ValidateCSIOperatorUpdate(ctx context.Context, platformClient platforminternalclient.PlatformInterface, new *platform.CSIOperator, old *platform.CSIOperator) field.ErrorList {
	allErrs := apiMachineryValidation.ValidateObjectMetaUpdate(&new.ObjectMeta, &old.ObjectMeta, field.NewPath("metadata"))
	allErrs = append(allErrs, ValidateCSIOperator(ctx, platformClient, new)...)
