...
import (
	...
	"tkestack.io/tke/pkg/platform/registry/gpumanager"
...
// NewStorage returns a Storage object that will work against gpu manager.
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, privilegedUsername string) *Storage {
	strategy := gpumanager.NewStrategy()
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &platform.GPUManager{} },
		NewListFunc:              func() runtime.Object { return &platform.GPUManagerList{} },
		DefaultQualifiedResource: platform.Resource("gpumanagers"),
		PredicateFunc:            gpumanager.MatchGPUManager,
	...
	statusStore := *store
	statusStore.UpdateStrategy = gpumanager.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = gpumanager.NewStatusStrategy(strategy)

	return &Storage{
		GPUManager: &REST{store, privilegedUsername},
		Status:     &StatusREST{&statusStore},
	}
}

// ValidateGetObjectAndTenantID validate name and tenantID, if success return GPUManager
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	gm := obj.(*platform.GPUManager)
	if err := util.FilterGPUManager(ctx, gm); err != nil {
		return nil, err
	}
	return gm, nil
}

// ValidateExportObjectAndTenantID validate name and tenantID, if success return GPUManager
...
// REST implements a RESTStorage for gpu manager against etcd.
type REST struct {
	*registry.Store
...
func (r *REST) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions, listOptions *metainternal.ListOptions) (runtime.Object, error) {
	if !authentication.IsAdministrator(ctx, r.privilegedUsername) {
		return nil, errors.NewMethodNotSupported(platform.Resource("gpumanagers"), "delete collection")
	}
	return r.Store.DeleteCollection(ctx, deleteValidation, options, listOptions)
}

// StatusREST implements the REST endpoint for changing the status of a gpumanager.
...
// New returns an empty object that can be used with CreateDaemonSet and UpdateDaemonSet after
