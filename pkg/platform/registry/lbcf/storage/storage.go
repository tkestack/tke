...
import (
	...
	"tkestack.io/tke/pkg/platform/registry/lbcf"
	"tkestack.io/tke/pkg/platform/util"
	"tkestack.io/tke/pkg/util/log"
)

// Storage includes storage for LBCF and all sub resources.
type Storage struct {
	LBCF   *REST
	Status *StatusREST
}

// NewStorage returns a Storage object that will work against LBCF.
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, platformClient platforminternalclient.PlatformInterface, privilegedUsername string) *Storage {
	strategy := lbcf.NewStrategy(platformClient)
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &platform.LBCF{} },
		NewListFunc:              func() runtime.Object { return &platform.LBCFList{} },
		DefaultQualifiedResource: platform.Resource("lbcfs"),
		PredicateFunc:            lbcf.MatchLBCF,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create LBCF etcd rest storage", log.Err(err))
	}

	statusStore := *store
	statusStore.UpdateStrategy = lbcf.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = lbcf.NewStatusStrategy(strategy)

	return &Storage{
		LBCF:   &REST{store, privilegedUsername},
		Status: &StatusREST{&statusStore},
	}
}

// ValidateGetObjectAndTenantID validate name and tenantID, if success return LBCF
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	hm := obj.(*platform.LBCF)
	if err := util.FilterLBCF(ctx, hm); err != nil {
		return nil, err
	}
	return hm, nil
}

// ValidateExportObjectAndTenantID validate name and tenantID, if success return LBCF
...
// REST implements a RESTStorage for LBCF against etcd.
type REST struct {
	*registry.Store
...
func (r *REST) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions, listOptions *metainternal.ListOptions) (runtime.Object, error) {
	if !authentication.IsAdministrator(ctx, r.privilegedUsername) {
		return nil, errors.NewMethodNotSupported(platform.Resource("lbcfs"), "delete collection")
	}
	return r.Store.DeleteCollection(ctx, deleteValidation, options, listOptions)
}

// StatusREST implements the REST endpoint for changing the status of a LBCF.
