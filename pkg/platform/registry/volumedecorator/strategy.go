...
func (Strategy) PrepareForCreate(ctx context.Context, obj runtime.Object) {
	...
	if len(tenantID) != 0 {
		volumeDecorator.Spec.TenantID = tenantID
	}

	if volumeDecorator.Name == "" && volumeDecorator.GenerateName == "" {
		volumeDecorator.GenerateName = "vd-"
	}

	if volumeDecorator.Spec.Version == "" {
		volumeDecorator.Spec.Version = images.LatestVersion
	}
}
...
func (Strategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	...
	if len(tenantID) != 0 {
		if oldDecorator.Spec.TenantID != tenantID {
			log.Panic("Unauthorized update new Decorator information", log.String("oldTenantID", oldDecorator.Spec.TenantID), log.String("newTenantID", newDecorator.Spec.TenantID), log.String("userTenantID", tenantID))
		}
		newDecorator.Spec.TenantID = tenantID
	}
	newDecorator.Status = oldDecorator.Status
}

// Validate validates a new tapp controller.
func (s *Strategy) Validate(ctx context.Context, obj runtime.Object) field.ErrorList {
	return ValidateVolumeDecorator(ctx, s.platformClient, obj.(*platform.VolumeDecorator))
}
...
func (s *Strategy) ValidateUpdate(ctx context.Context, obj, old runtime.Object) field.ErrorList {
	return ValidateVolumeDecoratorUpdate(ctx, s.platformClient, obj.(*platform.VolumeDecorator), old.(*platform.VolumeDecorator))
}

// GetAttrs returns labels and fields of a given object for filtering purposes.
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	decorator, _ := obj.(*platform.VolumeDecorator)
	return labels.Set(decorator.ObjectMeta.Labels), ToSelectableFields(decorator), nil
}

// MatchVolumeDecorator returns a generic matcher for a given label and field selector.
func MatchVolumeDecorator(label labels.Selector, field fields.Selector) storage.SelectionPredicate {
...
func ToSelectableFields(csiOperator *platform.VolumeDecorator) fields.Set {
...
func (StatusStrategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	newDecorator := obj.(*platform.VolumeDecorator)
	oldDecorator := old.(*platform.VolumeDecorator)
	newDecorator.Spec = oldDecorator.Spec
}
