...
import (
	...
	"tkestack.io/tke/pkg/platform/registry/template"
...
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, platformClient platforminternalclient.PlatformInterface, privilegedUsername string) *Storage {
	strategy := template.NewStrategy()
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &platform.Template{} },
		NewListFunc:              func() runtime.Object { return &platform.TemplateList{} },
		DefaultQualifiedResource: platform.Resource("templates"),
		PredicateFunc:            template.MatchTemplate,
	...
	statusStore := *store
	statusStore.UpdateStrategy = template.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = template.NewStatusStrategy(strategy)
...
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	m := obj.(*platform.Template)
	if err := util.FilterTemplate(ctx, m); err != nil {
		return nil, err
	}
	return m, nil
}
...
// REST implements a RESTStorage for template against etcd.
type REST struct {
	*registry.Store
...
func (r *REST) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions, listOptions *metainternal.ListOptions) (runtime.Object, error) {
	if !authentication.IsAdministrator(ctx, r.privilegedUsername) {
		return nil, apierrors.NewMethodNotSupported(platform.Resource("templates"), "delete collection")
	}
