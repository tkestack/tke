...
// Storage includes storage for ImageNamespace and all sub resources.
type Storage struct {
	ImageNamespace *REST
	Status         *StatusREST
...
// NewStorage returns a Storage object that will work against ImageNamespace sets.
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, businessClient *businessinternalclient.BusinessClient,
	...
	strategy := imagenamespace.NewStrategy(businessClient, registryClient)
	store := &registry.Store{
		NewFunc:                  func() runtime.Object { return &business.ImageNamespace{} },
		NewListFunc:              func() runtime.Object { return &business.ImageNamespaceList{} },
		DefaultQualifiedResource: business.Resource("imagenamespaces"),
		PredicateFunc:            imagenamespace.MatchNamespace,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create ImageNamespace etcd rest storage", log.Err(err))
	}

	statusStore := *store
	statusStore.UpdateStrategy = imagenamespace.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = imagenamespace.NewStatusStrategy(strategy)

	finalizeStore := *store
	finalizeStore.UpdateStrategy = imagenamespace.NewFinalizerStrategy(strategy)
	finalizeStore.ExportStrategy = imagenamespace.NewFinalizerStrategy(strategy)

	return &Storage{
		ImageNamespace: &REST{store, privilegedUsername},
		Status:         &StatusREST{&statusStore},
		Finalize:       &FinalizeREST{&finalizeStore},
	}
}

// ValidateGetObjectAndTenantID validate name and tenantID, if success return ImageNamespace
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	o := obj.(*business.ImageNamespace)
	if err := util.FilterImageNamespace(ctx, o); err != nil {
		return nil, err
	}
	return o, nil
}

// ValidateExportObjectAndTenantID validate name and tenantID, if success return ImageNamespace
...
// REST implements a RESTStorage for ImageNamespace sets against etcd.
type REST struct {
	*registry.Store
...
// Delete enforces life-cycle rules for ImageNamespace termination
func (r *REST) Delete(ctx context.Context, name string, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions) (runtime.Object, bool, error) {
	...
	ns := nsObj.(*business.ImageNamespace)
	...
	if options.Preconditions.UID == nil {
		...
		err = errors.NewConflict(
			business.Resource("imagenamespaces"),
			name,
	...
	if ns.DeletionTimestamp.IsZero() {
		...
		err = r.Store.Storage.GuaranteedUpdate(
			...
			storage.SimpleUpdate(func(existing runtime.Object) (runtime.Object, error) {
				existingNamespace, ok := existing.(*business.ImageNamespace)
				if !ok {
					// wrong type
					return nil, fmt.Errorf("expected *business.ImageNamespace, got %v", existing)
				}
				...
				// Set the ImageNamespace phase to terminating, if needed
				if existingNamespace.Status.Phase != business.ImageNamespaceTerminating {
					existingNamespace.Status.Phase = business.ImageNamespaceTerminating
				}

				// the current finalizers which are on ImageNamespace
				currentFinalizers := map[string]bool{}
				for _, f := range existingNamespace.Finalizers {
					currentFinalizers[f] = true
				}
				// the finalizers we should ensure on ImageNamespace
		...
		if err != nil {
			err = storageerr.InterpretGetError(err, business.Resource("imagenamespaces"), name)
			err = storageerr.InterpretUpdateError(err, business.Resource("imagenamespaces"), name)
	...
	if len(ns.Spec.Finalizers) != 0 {
		err = errors.NewConflict(business.Resource("imagenamespaces"), ns.Name,
			fmt.Errorf("the system is ensuring all content is removed from ImageNamespace. Upon completion, this ImageNamespace will automatically be purged by the system"))
		return nil, false, err
	}
...
func shouldDeleteDuringUpdate(ctx context.Context, key string, obj, existing runtime.Object) bool {
	ns, ok := obj.(*business.ImageNamespace)
	if !ok {
