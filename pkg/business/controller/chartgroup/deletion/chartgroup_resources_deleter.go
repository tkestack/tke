...
func (d *chartGroupResourcesDeleter) Delete(ctx context.Context, projectName, chartGroupName string) error {
	...
	chartGroup, err := d.businessClient.ChartGroups(projectName).Get(ctx, chartGroupName, metav1.GetOptions{})
	if err != nil {
...
func (d *chartGroupResourcesDeleter) deleteChartGroup(ctx context.Context, chartGroup *businessv1.ChartGroup) error {
	var opts metav1.DeleteOptions
	uid := chartGroup.UID
	if len(uid) > 0 {
		opts = metav1.DeleteOptions{Preconditions: &metav1.Preconditions{UID: &uid}}
	}
	err := d.businessClient.ChartGroups(chartGroup.Namespace).Delete(ctx, chartGroup.Name, opts)
...
type updateChartGroupFunc func(ctx context.Context, chartGroup *businessv1.ChartGroup) (*businessv1.ChartGroup, error)

// retryOnConflictError retries the specified fn if there was a conflict error
// it will return an error if the UID for an object changes across retry operations.
// TODO RetryOnConflict should be a generic concept in businessClient code
func (d *chartGroupResourcesDeleter) retryOnConflictError(ctx context.Context, chartGroup *businessv1.ChartGroup, fn updateChartGroupFunc) (result *businessv1.ChartGroup, err error) {
	...
	for {
		...
		latestChartGroup, err = d.businessClient.ChartGroups(latestChartGroup.Namespace).Get(ctx, latestChartGroup.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
...
func (d *chartGroupResourcesDeleter) updateChartGroupStatusFunc(ctx context.Context, chartGroup *businessv1.ChartGroup) (*businessv1.ChartGroup, error) {
	...
	return d.businessClient.ChartGroups(chartGroup.Namespace).UpdateStatus(ctx, &newChartGroup, metav1.UpdateOptions{})
}
...
func (d *chartGroupResourcesDeleter) finalizeChartGroup(ctx context.Context, chartGroup *businessv1.ChartGroup) (*businessv1.ChartGroup, error) {
...
type deleteResourceFunc func(ctx context.Context, deleter *chartGroupResourcesDeleter, chartGroup *businessv1.ChartGroup) error
...
func (d *chartGroupResourcesDeleter) deleteAllContent(ctx context.Context, chartGroup *businessv1.ChartGroup) error {
...
func updateChartGroup(ctx context.Context, deleter *chartGroupResourcesDeleter, chartGroup *businessv1.ChartGroup) error {
	chartGroupList, err := deleter.registryClient.ChartGroups().List(ctx, metav1.ListOptions{
	...
	pids := make([]string, 0)
	for _, projectID := range chartGroupObject.Spec.Projects {
		if projectID == chartGroup.Namespace {
			continue
		}
		pids = append(pids, projectID)
	}
	if len(chartGroupObject.Spec.Projects) != len(pids) {
		chartGroupObject.Spec.Projects = pids
		_, err = deleter.registryClient.ChartGroups().Update(ctx, &chartGroupObject, metav1.UpdateOptions{})
		if err != nil && !errors.IsNotFound(err) {
			return fmt.Errorf("updateChartGroup(%s), %s", chartGroup.Name, err)
		}
	}
	return nil
}
