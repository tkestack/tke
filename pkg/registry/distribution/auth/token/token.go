...
const (
	// Separator is the value which separates the header, claims, and
...
func NewToken(rawToken string) (*Token, error) {
	parts := strings.Split(rawToken, Separator)
	if len(parts) != 3 {
	...
	token.Raw = strings.Join(parts[:2], Separator)
...
func parseAndVerifyRawJWK(rawJWK *json.RawMessage, verifyOpts VerifyOptions) (pubKey libtrust.PublicKey, err error) {
	pubKey, err = libtrust.UnmarshalPublicKeyJWK(*rawJWK)
	if err != nil {
