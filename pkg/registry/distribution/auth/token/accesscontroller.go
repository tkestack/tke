...
func (ac authChallenge) challengeParams(r *http.Request) string {
	realm := fmt.Sprintf("%s://%s/%s", ac.scheme, r.Host, strings.TrimPrefix(ac.authPath, "/"))
...
func checkOptions(options map[string]interface{}) (tokenAccessOptions, error) {
	...
	keys := []string{"scheme", "authpath", "issuer", "service", "rootcertbundle"}
	...
	opts.scheme, opts.authPath, opts.issuer, opts.service, opts.rootCertBundle = vals[0], vals[1], vals[2], vals[3], vals[4]
...
func init() {
	if err := auth.Register("tkestack", newAccessController); err != nil {
		panic(err)
	}
}
