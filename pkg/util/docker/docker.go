...
// Docker represents an external docker command being prepared or run.
//
...
// Get docker images which match given image prefix
...
// Push a image which has a suffix about arch.
// Then create/amend local manifest of the image,
...
// For arm64 images, we need to add another two variants when annotating manifest
// variant: unknown, v8
func (d *Docker) PushArm64Variants(image string, manifestName string) error {
	for _, variant := range spec.Arm64Variants {
		variantImage := fmt.Sprintf("%s-%s", image, variant)
...
// Clear all local manifest lists
// Better to call before you want to create a manifest list
...
// Create a local manifest list. !IMPORTANT: local,local,local!
...
// Add additional information to a local image manifest. !IMPORTANT: local,local,local!
