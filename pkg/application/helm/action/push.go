...
// Push is the action for pulling a chart.
func (c *Client) Push(options *PushOptions) (existed bool, err error) {
	...
	// chart, err := helm.GetChartByName(options.ChartFile)
	// if err != nil {
	// 	return false, err
	// }
	...
	// log.Infof("Pushing %s to %s", filepath.Base(chartPackagePath), options.RepoURL)
	// resp, err := client.UploadChartPackage(chartPackagePath, options.ForceUpload)
