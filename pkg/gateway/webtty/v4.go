...
func newStreamProtocolV4(options remotecommand.StreamOptions) streamProtocolHandler {
...
func (d *errorDecoderV4) decode(message []byte) error {
	...
	case metav1.StatusFailure:
		if status.Reason == apimachinerycommand.NonZeroExitCodeReason {
			...
			for i := range status.Details.Causes {
				...
				if c.Type != apimachinerycommand.ExitCodeCauseType {
					continue
				}
			...
			return fmt.Errorf("error stream protocol error: no %s cause given", apimachinerycommand.ExitCodeCauseType)
		}
	default:
