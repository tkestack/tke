...
const commandDesc = `The TKE Auth provides a unified authentication adn authorization interface. In addition to 
service as a OIDC backend server and a local identity provider, it also provides 
the frontend to the policies and roles management.`

// NewApp creates a App object with default parameters.
func NewApp(basename string) *app.App {
	opts := options.NewOptions(basename)
	application := app.NewApp("Tencent Kubernetes Engine Auth",
		basename,
