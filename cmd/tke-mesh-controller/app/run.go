...
func Run(cfg *config.Config, stopCh <-chan struct{}) error {
	log.Info("Starting Tencent Kubernetes Engine mesh controller manager")
	...
	run := func(ctx context.Context) {
		rootClientBuilder := controller.SimpleControllerClientBuilder{
			ClientConfig: cfg.MeshAPIServerClientConfig,
		}
	...
	rl := resourcelock.NewMesh("tke-mesh-controller",
		cfg.LeaderElectionClient.MeshV1(),
		resourcelock.Config{
			Identity: id,
		})
