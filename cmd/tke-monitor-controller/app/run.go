...
// Run runs the specified monitor controller manager. This should never exit.
func Run(cfg *config.Config, stopCh <-chan struct{}) error {
	log.Info("Starting Tencent Kubernetes Engine monitor controller manager")
	...
	run := func(ctx context.Context) {
		rootClientBuilder := controller.SimpleControllerClientBuilder{
			ClientConfig: cfg.MonitorAPIServerClientConfig,
		}
	...
	rl := resourcelock.NewMonitor("tke-monitor-controller",
		cfg.LeaderElectionClient.MonitorV1(),
		resourcelock.Config{
			Identity: id,
		})
