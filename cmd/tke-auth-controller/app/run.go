...
// Run runs the specified auth controller manager. This should never exit.
func Run(cfg *config.Config, stopCh <-chan struct{}) error {
	log.Info("Starting Tencent Kubernetes Engine auth controller manager")
	...
	rl := resourcelock.NewAuth("tke-auth-controller",
		cfg.LeaderElectionClient.AuthV1(),
		resourcelock.Config{
			Identity: id,
		})
