...
func CreateControllerContext(cfg *config.Config, rootClientBuilder controller.ClientBuilder, stop <-chan struct{}) (ControllerContext, error) {
	applicationClient, err := versionedclientset.NewForConfig(rest.AddUserAgent(cfg.ApplicationAPIServerClientConfig, "tke-platform-controller"))
	if err != nil {
	...
	ctx := ControllerContext{
		...
		ApplicationClient:       applicationClient.ApplicationV1(),
	}
	return ctx, nil
}
