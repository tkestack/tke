...
// Options is the main context object for the TKE notify apiserver.
type Options struct {
	Log               *log.Options
...
func NewOptions(serverName string) *Options {
	return &Options{
		Log:               log.NewOptions(),
		SecureServing:     apiserveroptions.NewSecureServingOptions(serverName, 9457),
		Debug:             apiserveroptions.NewDebugOptions(),
		ETCD:              storageoptions.NewETCDStorageOptions("/tke/notify"),
...
// Should be called after tke-notify-api flags parsed.
