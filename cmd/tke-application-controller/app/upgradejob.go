...
func startUpgradeJobController(ctx ControllerContext) (http.Handler, bool, error) {
	if !ctx.AvailableResources[schema.GroupVersionResource{Group: applicationv1.GroupName, Version: "v1", Resource: "upgradejobs"}] {
		return nil, false, nil
	}

	// REGION_SET_FOR_JOB_CONTROLLER 环境变量为空时，不启动升级控制器
	region := os.Getenv("REGION_SET_FOR_JOB_CONTROLLER")
	if region == "" {
		return nil, false, nil
	}
