...
const commandDesc = `The Platform API server validates and configures data for the api objects which 
include clusters, machine, and others. The API Server services REST operations 
and provides the frontend to the project's shared state through which all other 
components interact.`

// NewApp creates a App object with default parameters.
func NewApp(basename string) *app.App {
	opts := options.NewOptions(basename)
	application := app.NewApp("Tencent Kubernetes Engine Platform API Server", basename,
