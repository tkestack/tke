...
const commandDesc = `The Application API server validates and configures data for the api objects which
 include helm release. The API Server services REST operations and
 provides the frontend to the release's shared state through which all other
 components interact.`

// NewApp creates a App object with default parameters.
func NewApp(basename string) *app.App {
	opts := options.NewOptions(basename)
	application := app.NewApp("Tencent Kubernetes Engine Authz API Server", basename,
