...
3. 单击【新建】，进入 “新建Workload” 页面。根据实际需求，设置 TApp 参数。关键参数信息如下，其中必填项为**工作负载名**、**实例内容器的名称和镜像**
    ...
    - **节点异常策略**
        ...
        - **迁移**：调度策略与 Deployment 一致，Pod 会迁移到新的节点
        - **不迁移**：调度策略与 StatefulSel 一致，异常 Pod 不会被迁移
   ...
   - **实例内容器**：根据实际需求，为 TApp 的Pod 设置一个或多个不同的容器，如下图所示：
   ...
   * **手动调节**：直接设定实例个数
      ...
      * **定时调节**：根据 [Crontab](https://baike.baidu.com/item/crontab) 语法周期性设置实例个数。前提：在[【扩展组件】](../../../../products/platform/extender.md)里安装 CronHPA
