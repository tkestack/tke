...
+ **高可用类型** ：高可用 VIP 地址。（**按需使用**）
  ...
  - **TKE提供**：用户只需提供可用的IP地址，TKE部署Keepalive，配置该IP为 Master 集群的 APIServer，以实现Global集群和控制台的高可用，此时该VIP和所有master节点IP地址都是APIserver地址。
...
- **定时调节**：根据Crontab 语法周期性设置实例个数。前提：在[扩展组件](../products/platform/extender.md)里安装CronHPA
- **显示高级设置**
...
- **服务访问方式**：选择是【仅在集群内部访问】该负载还是集群外部通过【主机端口访问】该负载
  ...
  - **Headless Service**：不创建用于集群内访问的ClusterIP，访问Service名称时返回后端Pods IP地址，用于适配自有的服务发现机制。解析Service域名时返回相应 Pod IP 而不是 Cluster IP。
