...
if [ ! -n "$1" ] || [ ! -n "$2" ] || [ ! -n "$3" ]; then
    echo "Usage: ./tools/helm.sh template|upgrade|install product1-demo ty 20190929101615"
    exit
fi
...
echo "helm -n default ${action} -f ../helmfile.d/config/product1-${env}/${release}.yaml --set bootstrap.image.tag=${tag} ${release} ${app}"
...
helm -n default ${action} -f ../helmfile.d/config/product1-${env}/${release}.yaml --set bootstrap.image.tag=${tag} ${release} ${app}



### Helm install example
#helm install product1-web-job1 product1-web -f product1-web/values/job1.yaml
