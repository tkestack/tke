...
export const editLogStashActions = {
  ...
  changeLogMode: (mode: string) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const { logStashName } = getState().logStashEdit;
      dispatch({
  ...
  selectContainerLogNamespace: (namespace: string, cIndex: number) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const { logStashEdit, route } = getState(),
        { containerLogs } = logStashEdit;
      const containerLogsArr: ContainerLogs[] = cloneDeep(containerLogs);
      ...
      const isCanFetchResourceList = namespaceSelection && workloadType ? true : false;
      dispatch(
   ...
   */
  initContainerLog: (isChangeNamespace = false, cIndex = 0) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const { logStashEdit } = getState();

      const containerLog: ContainerLogs = Object.assign({}, initContainerInputOption, { id: uuid() });
      // 这里需要去判断是否为切换namespace，切换nameespace只需要清空workloadSelection、workloadList、workloadListFetch这些信息
      if (isChangeNamespace) {
        const containerLogs = cloneDeep(logStashEdit.containerLogs);
        const editingContainerLog: ContainerLogs = containerLogs.find(item => item.status === 'editing');
  ...
  deleteContainerLog: (logIndex: number) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const containerLogs: ContainerLogs[] = cloneDeep(getState().logStashEdit.containerLogs);
  ...
  addContainerLog: () => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      ...
      // 上面剔除了已经选择namespace
      const editingIndex = containerLogsArr.findIndex(item => item.status === 'editing');
      if (editingIndex >= 0) {
        containerLogsArr[editingIndex]['status'] = 'edited';
      }

      // 添加新的containerLog到 containerLogs当中
      const newContainerLog = Object.assign({}, initContainerInputOption, {
        id: uuid(),
  ...
  selectContainerFileNamespace: (namespace: string) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      ...
      // 进行resourceList列表的拉取
      const { logStashEdit, route, clusterSelection } = getState();
      const { name } = clusterSelection[0].metadata;
      const { containerFileNamespace, containerFileWorkloadType } = logStashEdit;

      const isCanFetchResourceList = containerFileNamespace && containerFileWorkloadType ? true : false;
  ...
  selectContainerFileContainerName: (containerName: string, index: number) => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const { logStashEdit } = getState();
      const containerFilePathArr: ContainerFilePathItem[] = cloneDeep(logStashEdit.containerFilePaths);
  ...
  initContainerFileContainerFilePaths: () => {
    return async (dispatch: Redux.Dispatch, getState: GetState) => {
      const containerFilePathArr = [cloneDeep(initContainerFilePath)];
      dispatch({
        type: ActionType.UpdateContainerFilePaths,
        payload: containerFilePathArr
      });
