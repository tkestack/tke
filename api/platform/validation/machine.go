...
func ValidateMachineSpec(spec *platform.MachineSpec, fldPath *field.Path, platformClient internalversion.PlatformInterface) field.ErrorList {
	...
	allErrs = append(allErrs, ValidateMachineSpecType(spec, fldPath.Child("type"))...)
...
func ValidateMachineSpecType(spec *platform.MachineSpec, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}

	types := machineprovider.Providers()
	if spec.Type == "" {
		allErrs = append(allErrs, field.Required(fldPath, fmt.Sprintf("must specify one of %v", types)))
		return allErrs
	}
	if !funk.ContainsString(types, spec.Type) {
		allErrs = append(allErrs, field.NotSupported(fldPath, spec.Type, types))
	}
