...
func newRules(c *AuthClient) *rules {
	return &rules{
		client: c.RESTClient(),
	}
}
...
func (c *rules) Get(ctx context.Context, name string, options v1.GetOptions) (result *auth.Rule, err error) {
	result = &auth.Rule{}
	err = c.client.Get().
		Resource("rules").
		Name(name).
...
func (c *rules) List(ctx context.Context, opts v1.ListOptions) (result *auth.RuleList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &auth.RuleList{}
	err = c.client.Get().
		Resource("rules").
...
func (c *rules) Create(ctx context.Context, rule *auth.Rule, opts v1.CreateOptions) (result *auth.Rule, err error) {
	result = &auth.Rule{}
	err = c.client.Post().
		Resource("rules").
		VersionedParams(&opts, scheme.ParameterCodec).
...
func (c *rules) Update(ctx context.Context, rule *auth.Rule, opts v1.UpdateOptions) (result *auth.Rule, err error) {
	result = &auth.Rule{}
	err = c.client.Put().
		Resource("rules").
		Name(rule.Name).
...
func (c *rules) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *auth.Rule, err error) {
	result = &auth.Rule{}
