...
// List lists all LogAgents in the indexer.
func (s *logAgentLister) List(selector labels.Selector) (ret []*v1.LogAgent, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.LogAgent))
	})
	return ret, err
}

// Get retrieves the LogAgent from the index for a given name.
func (s *logAgentLister) Get(name string) (*v1.LogAgent, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("logagent"), name)
	}
	return obj.(*v1.LogAgent), nil
}
