...
type PrometheusLister interface {
	...
	List(selector labels.Selector) (ret []*monitor.Prometheus, err error)
	// Get retrieves the Prometheus from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*monitor.Prometheus, error)
...
// List lists all Prometheuses in the indexer.
func (s *prometheusLister) List(selector labels.Selector) (ret []*monitor.Prometheus, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*monitor.Prometheus))
	})
	return ret, err
}

// Get retrieves the Prometheus from the index for a given name.
func (s *prometheusLister) Get(name string) (*monitor.Prometheus, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(monitor.Resource("prometheus"), name)
	}
	return obj.(*monitor.Prometheus), nil
}
