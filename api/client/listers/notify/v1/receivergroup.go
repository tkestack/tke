...
// List lists all ReceiverGroups in the indexer.
func (s *receiverGroupLister) List(selector labels.Selector) (ret []*v1.ReceiverGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ReceiverGroup))
	})
	return ret, err
}

// Get retrieves the ReceiverGroup from the index for a given name.
func (s *receiverGroupLister) Get(name string) (*v1.ReceiverGroup, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("receivergroup"), name)
	}
	return obj.(*v1.ReceiverGroup), nil
}
