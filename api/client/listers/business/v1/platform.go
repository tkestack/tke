...
// List lists all Platforms in the indexer.
func (s *platformLister) List(selector labels.Selector) (ret []*v1.Platform, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Platform))
	})
	return ret, err
}

// Get retrieves the Platform from the index for a given name.
func (s *platformLister) Get(name string) (*v1.Platform, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("platform"), name)
	}
	return obj.(*v1.Platform), nil
}
