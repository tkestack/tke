...
// List lists all Portals in the indexer.
func (s *portalLister) List(selector labels.Selector) (ret []*v1.Portal, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Portal))
	})
	return ret, err
}

// Get retrieves the Portal from the index for a given name.
func (s *portalLister) Get(name string) (*v1.Portal, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("portal"), name)
	}
	return obj.(*v1.Portal), nil
}
