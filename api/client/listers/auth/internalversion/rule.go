...
// List lists all Rules in the indexer.
func (s *ruleLister) List(selector labels.Selector) (ret []*auth.Rule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*auth.Rule))
	})
	return ret, err
}

// Get retrieves the Rule from the index for a given name.
func (s *ruleLister) Get(name string) (*auth.Rule, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(auth.Resource("rule"), name)
	}
	return obj.(*auth.Rule), nil
}
