...
// List lists all ProjectPolicyBindings in the indexer.
func (s *projectPolicyBindingLister) List(selector labels.Selector) (ret []*auth.ProjectPolicyBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*auth.ProjectPolicyBinding))
	})
	return ret, err
}

// Get retrieves the ProjectPolicyBinding from the index for a given name.
func (s *projectPolicyBindingLister) Get(name string) (*auth.ProjectPolicyBinding, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(auth.Resource("projectpolicybinding"), name)
	}
	return obj.(*auth.ProjectPolicyBinding), nil
}
