...
type APISigningKeyLister interface {
	...
	List(selector labels.Selector) (ret []*auth.APISigningKey, err error)
	// Get retrieves the APISigningKey from the index for a given name.
	Get(name string) (*auth.APISigningKey, error)
...
// List lists all APISigningKeys in the indexer.
func (s *aPISigningKeyLister) List(selector labels.Selector) (ret []*auth.APISigningKey, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*auth.APISigningKey))
	})
	return ret, err
}

// Get retrieves the APISigningKey from the index for a given name.
func (s *aPISigningKeyLister) Get(name string) (*auth.APISigningKey, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(auth.Resource("apisigningkey"), name)
	}
	return obj.(*auth.APISigningKey), nil
}
