...
type PolicyLister interface {
	...
	List(selector labels.Selector) (ret []*auth.Policy, err error)
	// Get retrieves the Policy from the index for a given name.
	Get(name string) (*auth.Policy, error)
...
// List lists all Policies in the indexer.
func (s *policyLister) List(selector labels.Selector) (ret []*auth.Policy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*auth.Policy))
	})
	return ret, err
}

// Get retrieves the Policy from the index for a given name.
func (s *policyLister) Get(name string) (*auth.Policy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(auth.Resource("policy"), name)
	}
	return obj.(*auth.Policy), nil
}
