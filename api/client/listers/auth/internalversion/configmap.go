...
type ConfigMapLister interface {
	...
	List(selector labels.Selector) (ret []*auth.ConfigMap, err error)
	// Get retrieves the ConfigMap from the index for a given name.
	Get(name string) (*auth.ConfigMap, error)
...
// List lists all ConfigMaps in the indexer.
func (s *configMapLister) List(selector labels.Selector) (ret []*auth.ConfigMap, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*auth.ConfigMap))
	})
	return ret, err
}

// Get retrieves the ConfigMap from the index for a given name.
func (s *configMapLister) Get(name string) (*auth.ConfigMap, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(auth.Resource("configmap"), name)
	}
	return obj.(*auth.ConfigMap), nil
}
