...
type CustomPolicyBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CustomPolicyBinding, err error)
...
// List lists all CustomPolicyBindings in the indexer.
func (s *customPolicyBindingLister) List(selector labels.Selector) (ret []*v1.CustomPolicyBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CustomPolicyBinding))
	})
	return ret, err
}
...
type CustomPolicyBindingNamespaceLister interface {
	...
	Get(name string) (*v1.CustomPolicyBinding, error)
...
// List lists all CustomPolicyBindings in the indexer for a given namespace.
func (s customPolicyBindingNamespaceLister) List(selector labels.Selector) (ret []*v1.CustomPolicyBinding, err error) {
...
// Get retrieves the CustomPolicyBinding from the indexer for a given namespace and name.
func (s customPolicyBindingNamespaceLister) Get(name string) (*v1.CustomPolicyBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("custompolicybinding"), name)
	}
	return obj.(*v1.CustomPolicyBinding), nil
}
