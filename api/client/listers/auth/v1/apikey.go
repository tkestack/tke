...
type APIKeyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.APIKey, err error)
	// Get retrieves the APIKey from the index for a given name.
	Get(name string) (*v1.APIKey, error)
...
// List lists all APIKeys in the indexer.
func (s *aPIKeyLister) List(selector labels.Selector) (ret []*v1.APIKey, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.APIKey))
	})
	return ret, err
}

// Get retrieves the APIKey from the index for a given name.
func (s *aPIKeyLister) Get(name string) (*v1.APIKey, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("apikey"), name)
	}
	return obj.(*v1.APIKey), nil
}
