...
type MeshManagerLister interface {
	...
	List(selector labels.Selector) (ret []*mesh.MeshManager, err error)
	// Get retrieves the MeshManager from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*mesh.MeshManager, error)
...
// List lists all MeshManagers in the indexer.
func (s *meshManagerLister) List(selector labels.Selector) (ret []*mesh.MeshManager, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mesh.MeshManager))
	})
	return ret, err
}

// Get retrieves the MeshManager from the index for a given name.
func (s *meshManagerLister) Get(name string) (*mesh.MeshManager, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mesh.Resource("meshmanager"), name)
	}
	return obj.(*mesh.MeshManager), nil
}
