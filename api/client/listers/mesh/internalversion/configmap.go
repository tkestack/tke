...
type ConfigMapLister interface {
	...
	List(selector labels.Selector) (ret []*mesh.ConfigMap, err error)
	// Get retrieves the ConfigMap from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*mesh.ConfigMap, error)
...
// List lists all ConfigMaps in the indexer.
func (s *configMapLister) List(selector labels.Selector) (ret []*mesh.ConfigMap, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mesh.ConfigMap))
	})
	return ret, err
}

// Get retrieves the ConfigMap from the index for a given name.
func (s *configMapLister) Get(name string) (*mesh.ConfigMap, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mesh.Resource("configmap"), name)
	}
	return obj.(*mesh.ConfigMap), nil
}
