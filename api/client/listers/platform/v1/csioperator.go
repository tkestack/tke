...
// List lists all CSIOperators in the indexer.
func (s *cSIOperatorLister) List(selector labels.Selector) (ret []*v1.CSIOperator, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CSIOperator))
	})
	return ret, err
}

// Get retrieves the CSIOperator from the index for a given name.
func (s *cSIOperatorLister) Get(name string) (*v1.CSIOperator, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("csioperator"), name)
	}
	return obj.(*v1.CSIOperator), nil
}
