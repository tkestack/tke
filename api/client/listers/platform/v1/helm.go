...
type HelmLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Helm, err error)
	// Get retrieves the Helm from the index for a given name.
	Get(name string) (*v1.Helm, error)
	HelmListerExpansion
}
...
// List lists all Helms in the indexer.
func (s *helmLister) List(selector labels.Selector) (ret []*v1.Helm, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Helm))
	})
	return ret, err
}

// Get retrieves the Helm from the index for a given name.
func (s *helmLister) Get(name string) (*v1.Helm, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("helm"), name)
	}
	return obj.(*v1.Helm), nil
}
