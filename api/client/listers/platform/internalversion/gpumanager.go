...
type GPUManagerLister interface {
	...
	List(selector labels.Selector) (ret []*platform.GPUManager, err error)
	// Get retrieves the GPUManager from the index for a given name.
	Get(name string) (*platform.GPUManager, error)
...
// List lists all GPUManagers in the indexer.
func (s *gPUManagerLister) List(selector labels.Selector) (ret []*platform.GPUManager, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.GPUManager))
	})
	return ret, err
}

// Get retrieves the GPUManager from the index for a given name.
func (s *gPUManagerLister) Get(name string) (*platform.GPUManager, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("gpumanager"), name)
	}
	return obj.(*platform.GPUManager), nil
}
