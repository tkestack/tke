...
type TemplateLister interface {
	...
	List(selector labels.Selector) (ret []*platform.Template, err error)
	// Get retrieves the Template from the index for a given name.
	Get(name string) (*platform.Template, error)
...
// List lists all Templates in the indexer.
func (s *templateLister) List(selector labels.Selector) (ret []*platform.Template, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.Template))
	})
	return ret, err
}

// Get retrieves the Template from the index for a given name.
func (s *templateLister) Get(name string) (*platform.Template, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("template"), name)
	}
	return obj.(*platform.Template), nil
}
